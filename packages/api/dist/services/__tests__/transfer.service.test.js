...
describe('TransferService', () => {
    ...
    beforeEach(() => {
        ...
        mockPaymentService = transferService.paymentService;
        mockWalletService = transferService.walletService;
        mockPayoutService = transferService.payoutService;
    ...
    describe('createTransfer', () => {
        it('should create a successful transfer with all steps', async () => {
            ...
            expect(result.status).toBe(transfer_service_1.TransferStatus.PAYING_OUT);
    ...
    describe('getTransferStatus', () => {
        ...
        it('should update status from external services', async () => {
            ...
            mockPaymentService.waitForPaymentConfirmation.mockImplementation(() => new Promise(() => { }) // Never resolves to keep in PAYMENT_PROCESSING
            );
    ...
    describe('cancelTransfer', () => {
        it('should cancel transfer in early stages', async () => {
            ...
            mockPaymentService.waitForPaymentConfirmation.mockImplementation(() => new Promise(() => { }) // Never resolves
            );
            const transferPromise = transferService.createTransfer(mockTransferRequest);
            ...
            if (transferId) {
                ...
                const status = await transferService.getTransferStatus(transferId);
                expect(status.status).toBe(transfer_service_1.TransferStatus.FAILED);
            }
            // Clean up the hanging promise
            transferPromise.catch(() => { }); // Ignore the error
        });
        it('should not cancel transfer in later stages', async () => {
            ...
            expect(transfer.status).toBe(transfer_service_1.TransferStatus.PAYING_OUT);
