...
describe('FXService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        fxService = new fx_service_1.FXService();
        // Clear any cached data
        fxService.rateCache.clear();
        fxService.lockedRates.clear();
    });
    describe('getExchangeRate', () => {
        ...
        it('should handle API failures gracefully with fallback rates', async () => {
            ...
            jest.spyOn(fxService, 'mockCircleRateAPI').mockRejectedValue(new Error('API unavailable'));
            const request = {
                fromCurrency: 'USD',
                toCurrency: 'EUR'
            };
    ...
    describe('getRateUpdates', () => {
        it('should provide real-time rate updates', async () => {
            const rates = [];
            const callback = (rate) => rates.push(rate);
    ...
    describe('fee calculations', () => {
        it('should calculate fees correctly', async () => {
            const fees = fxService.calculateFees(100);
        ...
        it('should calculate all transfer fees correctly', async () => {
            const fees = fxService.calculateAllFees(100);
    ...
    describe('caching behavior', () => {
        it('should cache rates for performance', async () => {
            ...
            const apiSpy = jest.spyOn(fxService, 'mockCircleRateAPI');
        ...
        it('should refresh cache when expired', async () => {
            ...
            const cachedRate = fxService.rateCache.get(cacheKey);
            if (cachedRate) {
    ...
    describe('edge cases and error handling', () => {
        ...
        it('should clean up expired locked rates', async () => {
            ...
            expect(fxService.lockedRates.size).toBe(1);
            // Wait for expiration
            await new Promise(resolve => setTimeout(resolve, 100));
            ...
            expect(fxService.lockedRates.size).toBe(0);
        });
    });
});
