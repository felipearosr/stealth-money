...
describe('TransferService - Method Selection Logic', () => {
    let transferService;
    let mockMantleService;
    beforeEach(() => {
        // Reset all mocks
        jest.clearAllMocks();
        transferService = new transfer_service_1.TransferService();
        mockMantleService = transferService.mantleService;
    ...
    describe('calculateTransfer', () => {
        it('should return both Circle and Mantle options when both are available', async () => {
            ...
            expect(result.options.find(o => o.method === transfer_service_1.TransferMethod.CIRCLE)).toBeDefined();
            expect(result.options.find(o => o.method === transfer_service_1.TransferMethod.MANTLE)).toBeDefined();
        ...
        it('should only return Circle option when Mantle is disabled', async () => {
            ...
            expect(result.options[0].method).toBe(transfer_service_1.TransferMethod.CIRCLE);
        });
    ...
    describe('createMantleTransfer', () => {
        it('should create a Mantle transfer successfully', async () => {
            ...
            expect(result.metadata?.transferMethod).toBe(transfer_service_1.TransferMethod.MANTLE);
    ...
    describe('cost comparison', () => {
        it('should calculate cost differences between Circle and Mantle', async () => {
            ...
            const result = await transferService.calculateTransfer(request);
            const circleOption = result.options.find(o => o.method === transfer_service_1.TransferMethod.CIRCLE);
            const mantleOption = result.options.find(o => o.method === transfer_service_1.TransferMethod.MANTLE);
        ...
        it('should show correct fee breakdown for each method', async () => {
            ...
            result.options.forEach(option => {
                ...
                expect(option.totalCost).toBe(option.fees.processing + option.fees.network + option.fees.exchange);
            });
        });
    });
    ...
    describe('unified transfer result interface', () => {
        it('should return consistent transfer result structure for both methods', async () => {
            ...
            const mockPaymentService = transferService.paymentService;
            const mockWalletService = transferService.walletService;
            const mockPayoutService = transferService.payoutService;
            ...
            expect(mantleResult.metadata?.transferMethod).toBe(transfer_service_1.TransferMethod.MANTLE);
        });
    });
});
