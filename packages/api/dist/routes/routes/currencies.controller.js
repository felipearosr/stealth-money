 ...
 * GET /api/currencies
 */
router.get('/', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const currencies = currency_config_service_1.CurrencyConfigService.getSupportedCurrencies();
        const sendCurrencies = currency_config_service_1.CurrencyConfigService.getSendCurrencies();
        const receiveCurrencies = currency_config_service_1.CurrencyConfigService.getReceiveCurrencies();
        res.json({
 ...
 */
router.get('/pairs', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const pairs = currency_config_service_1.CurrencyConfigService.getSupportedCurrencyPairs();
        res.json({
 ...
 */
router.get('/health', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const healthCheck = currency_config_service_1.CurrencyConfigService.healthCheck();
 ...
 */
router.get('/:code', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const currencySchema = zod_1.z.object({
            code: zod_1.z.string().length(3).regex(/^[A-Z]{3}$/)
        });
        const { code } = currencySchema.parse({
            code: req.params.code.toUpperCase()
        });
        const currency = currency_config_service_1.CurrencyConfigService.getCurrency(code);
        if (!currency) {
            return res.status(404).json({
                error: 'CURRENCY_NOT_FOUND',
                message: `Currency ${code} is not supported`,
                supportedCurrencies: currency_config_service_1.CurrencyConfigService.getSendCurrencies().concat(currency_config_service_1.CurrencyConfigService.getReceiveCurrencies())
            });
        }
        const sendCurrencies = currency_config_service_1.CurrencyConfigService.getSendCurrencies();
        const receiveCurrencies = currency_config_service_1.CurrencyConfigService.getReceiveCurrencies();
        res.json({
            ...
            canSend: sendCurrencies.includes(currency.code),
            canReceive: receiveCurrencies.includes(currency.code),
            displayName: currency_config_service_1.CurrencyConfigService.getCurrencyDisplayName(currency.code),
            lastUpdated: new Date().toISOString()
        });
    }
    catch (error) {
 ...
 */
router.post('/validate', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        ...
        const validationError = currency_config_service_1.CurrencyConfigService.validateCurrencyPair(sendCurrency, receiveCurrency, amount);
        if (validationError) {
            return res.status(400).json({
                isValid: false,
        ...
        const pair = currency_config_service_1.CurrencyConfigService.getCurrencyPair(sendCurrency, receiveCurrency);
        res.json({
            isValid: true,
            pair: pair ? {
                from: pair.from,
 ...
 */
router.get('/rates/:from/:to', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const rateSchema = zod_1.z.object({
            from: zod_1.z.string().length(3).regex(/^[A-Z]{3}$/),
            to: zod_1.z.string().length(3).regex(/^[A-Z]{3}$/)
        });
        ...
        if (!currency_config_service_1.CurrencyConfigService.isCurrencyPairSupported(from, to)) {
            return res.status(400).json({
                error: 'UNSUPPORTED_CURRENCY_PAIR',
                message: `Currency pair ${from} to ${to} is not supported`,
                supportedPairs: currency_config_service_1.CurrencyConfigService.getSupportedCurrencyPairs().map(p => `${p.from}-${p.to}`)
            });
        }
        const fxService = new fx_service_1.FXService();
...
router.post('/rates/lock', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        ...
        // Validate currency pair
        const validationError = currency_config_service_1.CurrencyConfigService.validateCurrencyPair(fromCurrency, toCurrency, amount);
        if (validationError) {
            return res.status(400).json({
                error: 'VALIDATION_FAILED',
                message: validationError
 ...
 */
router.get('/rates/:from/:to/history', security_middleware_1.generalRateLimit, async (req, res) => {
    try {
        const historySchema = zod_1.z.object({
            from: zod_1.z.string().length(3).regex(/^[A-Z]{3}$/),
            to: zod_1.z.string().length(3).regex(/^[A-Z]{3}$/),
            hours: zod_1.z.string().regex(/^\d+$/).transform(Number).optional().default("24")
        });
