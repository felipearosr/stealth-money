...
function validateBankAccountData(currency, data) {
    const errors = [];
    ...
    try {
        ...
        const searchResults = users.map((user) => ({
            ...
            supportedCurrencies: (user.bankAccounts || [])
                .filter((account) => account.isVerified && account.isActive)
        ...
        logging_middleware_1.logger.info('User search completed', {
            query: query.trim(),
        ...
        logging_middleware_1.logger.error('User search failed', {
...
router.get('/me/recipients', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const recipients = await dbService.getUserSavedRecipients(authReq.userId);
        logging_middleware_1.logger.info('Retrieved user recipients', {
        ...
        logging_middleware_1.logger.error('Failed to retrieve recipients', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId
        });
        res.status(500).json({
...
router.post('/me/recipients', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        ...
        logging_middleware_1.logger.info('Created saved recipient', {
            userId: authReq.userId,
        ...
        logging_middleware_1.logger.error('Failed to create recipient', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId
        });
        res.status(500).json({
...
router.put('/me/recipients/:id', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const { id } = req.params;
        ...
        logging_middleware_1.logger.info('Updated saved recipient', {
            userId: authReq.userId,
            recipientId: id
        });
        res.json(recipient);
        ...
        logging_middleware_1.logger.error('Failed to update recipient', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId,
...
router.delete('/me/recipients/:id', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const { id } = req.params;
        await dbService.deleteSavedRecipient(id);
        logging_middleware_1.logger.info('Deleted saved recipient', {
            userId: authReq.userId,
            recipientId: id
        });
        res.status(204).send();
        ...
        logging_middleware_1.logger.error('Failed to delete recipient', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId,
            recipientId: req.params.id
        });
...
router.get('/me/bank-accounts', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const bankAccounts = await dbService.getUserBankAccounts(authReq.userId);
        logging_middleware_1.logger.info('Retrieved user bank accounts', {
            userId: authReq.userId,
        ...
        logging_middleware_1.logger.error('Failed to retrieve bank accounts', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId
        });
        res.status(500).json({
...
router.post('/me/bank-accounts', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        ...
        logging_middleware_1.logger.info('Created bank account', {
            userId: authReq.userId,
        ...
        logging_middleware_1.logger.error('Failed to create bank account', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId
        });
        res.status(500).json({
...
router.put('/me/bank-accounts/:id', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const { id } = req.params;
        const updateData = req.body;
        ...
        logging_middleware_1.logger.info('Updated bank account', {
            userId: authReq.userId,
            accountId: id
        });
        ...
        logging_middleware_1.logger.error('Failed to update bank account', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId,
...
router.delete('/me/bank-accounts/:id', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        const { id } = req.params;
        // Get the existing account to check ownership
        ...
        logging_middleware_1.logger.info('Deleted bank account', {
            userId: authReq.userId,
            accountId: id
        });
        res.status(204).send();
        ...
        logging_middleware_1.logger.error('Failed to delete bank account', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId,
            accountId: req.params.id
        });
...
router.get('/me/supported-currencies', auth_middleware_1.requireAuth, user_sync_middleware_1.syncUser, async (req, res) => {
    const authReq = req;
    try {
        ...
        logging_middleware_1.logger.error('Failed to retrieve supported currencies', {
            error: error instanceof Error ? error.message : 'Unknown error',
            userId: authReq.userId
        });
        res.status(500).json({
