...
class BlockchainService {
    ...
    async initializeRealBlockchain() {
        try {
            console.log(`🔗 Initializing real blockchain connection to ${this.config.networkName}...`);
            this.provider = new ethers_1.ethers.JsonRpcProvider(this.config.nodeProviderUrl);
            ...
            this.transferManagerContract = TransferManager__factory.connect(this.config.contractAddress, this.wallet);
            // Test the connection
     ...
     */
    async releaseFunds(recipient, amountInStablecoin, transactionId) {
        if (this.config.mode === 'mock') {
        ...
        try {
            ...
            const amountFormatted = ethers_1.ethers.formatUnits(amountInStablecoin, 6); // Assuming USDC with 6 decimals
            ...
            let gasEstimate;
            try {
                gasEstimate = await this.transferManagerContract.release.estimateGas(recipient, amountInStablecoin, transactionIdBytes32);
            ...
            console.log(`   Gas price: ${ethers_1.ethers.formatUnits(gasPrice, 'gwei')} gwei`);
            // Check wallet balance for gas
            const walletBalance = await this.provider.getBalance(this.wallet.address);
            const estimatedGasCost = gasLimit * gasPrice;
            if (walletBalance < estimatedGasCost) {
                throw new Error(`Insufficient ETH for gas. Required: ${ethers_1.ethers.formatEther(estimatedGasCost)} ETH, Available: ${ethers_1.ethers.formatEther(walletBalance)} ETH`);
            }
            // Execute the transaction
            console.log('📤 Sending transaction...');
            const tx = await this.transferManagerContract.release(recipient, amountInStablecoin, transactionIdBytes32, {
                gasLimit,
                gasPrice
            });
            ...
            const receipt = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Transaction confirmation timeout')), 300000) // 5 minute timeout
                )
            ]);
            if (!receipt) {
     ...
     */
    async waitForConfirmations(txHash, requiredConfirmations = 1) {
        if (this.config.mode === 'mock') {
