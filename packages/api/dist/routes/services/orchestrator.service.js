...
class OrchestratorService {
     ...
     */
    async handleSuccessfulPayment(transactionId) {
    ...
    async executeBlockchainRelease(transaction) {
        try {
            ...
            const releaseResult = await this.blockchainService.releaseFunds(recipientAddress, amountInTokenUnits.toString(), transaction.id);
            ...
            if (requiredConfirmations > 0) {
                const confirmed = await this.blockchainService.waitForConfirmations(releaseResult.txHash, requiredConfirmations);
                if (!confirmed) {
                    return {
                        success: false,
