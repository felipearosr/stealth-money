...
describe('MantleService - Wallet Management', () => {
  ...
  describe('createWalletFromMnemonic', () => {
    ...
    it('should create wallet from valid mnemonic', async () => {
      ...
      expect(mockEthers.HDNodeWallet.fromPhrase).toHaveBeenCalledWith(
        validMnemonic, 
        undefined, 
        "m/44'/60'/0'/0/0"
      );
  ...
  describe('getWalletBalance', () => {
    ...
    it('should handle USD price fetch errors gracefully', async () => {
      ...
      jest.spyOn(mantleService as any, 'getMNTPriceUSD').mockRejectedValue(new Error('Price fetch failed'));
