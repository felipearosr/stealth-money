...
describe('NotificationService', () => {
  ...
  const mockTransferContext: TransferContext = {
  ...
  describe('sendNotification', () => {
    ...
    it('should send browser notification successfully', async () => {
      ...
      expect(results).toHaveLength(1);
      expect(results[0].status).toBe('sent');
      expect(results[0].channel).toBe(NotificationChannel.BROWSER);
      expect(results[0].sentAt).toBeInstanceOf(Date);
    });

    it('should send webhook notification successfully', async () => {
      const request: NotificationRequest = {
        type: NotificationType.TRANSFER_PROCESSING,
    ...
    it('should send multiple channel notifications', async () => {
      const request: NotificationRequest = {
        type: NotificationType.TRANSFER_INITIATED,
        channels: [NotificationChannel.EMAIL, NotificationChannel.SMS, NotificationChannel.BROWSER],
        recipient: mockRecipient,
        context: mockTransferContext
      };
    ...
    it('should handle unsupported notification channel', async () => {
      const request: NotificationRequest = {
        type: NotificationType.TRANSFER_INITIATED,
        channels: ['unsupported' as NotificationChannel],
        recipient: mockRecipient,
        context: mockTransferContext
      };
  ...
  describe('notifyTransferEvent', () => {
    it('should notify multiple recipients for transfer event', async () => {
      ...
      const results = await notificationService.notifyTransferEvent(
      ...
      expect(results.every(r => r.status === 'sent')).toBe(true);
      expect(results.every(r => r.channel === NotificationChannel.EMAIL)).toBe(true);
    });

    it('should set high priority for failed transfers', async () => {
      const consoleSpy = jest.spyOn(console, 'log').mockImplementation();

      await notificationService.notifyTransferEvent(
        NotificationType.TRANSFER_FAILED,
        { ...mockTransferContext, errorMessage: 'Payment declined' },
        [mockRecipient],
        [NotificationChannel.EMAIL]
      );
  ...
  describe('error handling', () => {
    ...
    it('should continue processing other channels when one fails', async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.find(r => r.channel === NotificationChannel.EMAIL)?.status).toBe('sent');
      expect(results.find(r => r.channel === NotificationChannel.SMS)?.status).toBe('failed');
    });
  });
  ...
  describe('notification content validation', () => {
    ...
    it('should handle compliance hold notifications', async () => {
      ...
      const request: NotificationRequest = {
        type: NotificationType.COMPLIANCE_HOLD,
        channels: [NotificationChannel.EMAIL],
        recipient: mockRecipient,
        context: mockTransferContext
      };
