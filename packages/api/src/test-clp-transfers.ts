...
async function testCLPTransfers() {
  ...
  try {
    ...
    const validation1 = CurrencyConfigService.validateCurrencyPair('CLP', 'CLP', 10000);
    console.log('CLP-to-CLP validation (10,000 CLP):', validation1 || 'Valid');

    const validation2 = CurrencyConfigService.validateCurrencyPair('CLP', 'CLP', 500);
    console.log('CLP-to-CLP validation (500 CLP - below minimum):', validation2 || 'Valid');

    const validation3 = CurrencyConfigService.validateCurrencyPair('CLP', 'CLP', 50000000);
