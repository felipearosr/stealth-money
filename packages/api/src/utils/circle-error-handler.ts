...
export class CircleError extends Error {
  public readonly code: CircleErrorCode;
 ...
 */
export class CircleErrorHandler {
  ...
  static handleError(error: any, operation: string): never {
    ...
    if (error.message?.includes('Invalid') || error.message?.includes('required')) {
      throw new CircleError(
        CircleErrorCode.VALIDATION_ERROR,
   ...
   */
  private static handleCircleApiError(errorData: any, operation: string, originalError: any): never {
      ...
      switch (errorCode) {
        ...
        case 'payment_declined':
          throw new CircleError(
            ...
            `Payment declined: ${errorMessage}`,
            'Your payment was declined. Please check your card details and try again, or use a different payment method.',
            false,
            undefined,
            { errorCode, originalMessage: errorMessage },
            originalError
          );
        
        case 'insufficient_funds':
          throw new CircleError(
            CircleErrorCode.INSUFFICIENT_FUNDS,
            `Insufficient funds: ${errorMessage}`,
            'Your card has insufficient funds for this transaction. Please use a different payment method.',
            false,
            undefined,
            { errorCode, originalMessage: errorMessage },
            originalError
        ...
        case 'card_expired':
          throw new CircleError(
            CircleErrorCode.CARD_EXPIRED,
            `Card expired: ${errorMessage}`,
        ...
        case 'three_d_secure_failed':
          throw new CircleError(
            CircleErrorCode.THREE_D_SECURE_FAILED,
            `3D Secure authentication failed: ${errorMessage}`,
            'Card authentication failed. Please try again or use a different payment method.',
            true,
            60,
            { errorCode, originalMessage: errorMessage },
            originalError
    ...
    if (operation.includes('wallet')) {
      switch (errorCode) {
        case 'wallet_not_found':
          throw new CircleError(
            CircleErrorCode.WALLET_NOT_FOUND,
            `Wallet not found: ${errorMessage}`,
        ...
        case 'wallet_frozen':
          throw new CircleError(
            CircleErrorCode.WALLET_FROZEN,
            `Wallet frozen: ${errorMessage}`,
        ...
        case 'insufficient_balance':
          throw new CircleError(
            CircleErrorCode.INSUFFICIENT_WALLET_BALANCE,
            `Insufficient wallet balance: ${errorMessage}`,
    ...
    if (operation.includes('payout')) {
      switch (errorCode) {
        ...
        case 'invalid_bic':
          throw new CircleError(
            CircleErrorCode.INVALID_BANK_DETAILS,
            `Invalid bank details: ${errorMessage}`,
            'The bank account details are invalid. Please check and correct the information.',
            false,
            undefined,
            { errorCode, originalMessage: errorMessage },
            originalError
        ...
        case 'bank_rejected':
          throw new CircleError(
            CircleErrorCode.BANK_REJECTED,
            `Bank rejected transfer: ${errorMessage}`,
            'The receiving bank rejected the transfer. Please verify the account details and try again.',
            true,
            300, // 5 minutes
            { errorCode, originalMessage: errorMessage },
            originalError
        ...
        case 'compliance_hold':
          throw new CircleError(
            CircleErrorCode.COMPLIANCE_HOLD,
            `Transfer under compliance review: ${errorMessage}`,
            'Your transfer is under review for compliance. You will be notified once the review is complete.',
            false,
            undefined,
            { errorCode, originalMessage: errorMessage },
            originalError
    ...
    if (errorCode === 'rate_limited' || errorCode === 'too_many_requests') {
      throw new CircleError(
        CircleErrorCode.API_RATE_LIMITED,
    ...
    if (errorCode === 'unauthorized' || errorCode === 'authentication_failed') {
      throw new CircleError(
        CircleErrorCode.AUTHENTICATION_FAILED,
    ...
    throw new CircleError(
      CircleErrorCode.UNKNOWN_ERROR,
      `Circle API error during ${operation}: ${errorMessage}`,
      'A payment service error occurred. Please try again or contact support if the problem persists.',
      true,
      30,
      { errorCode, originalMessage: errorMessage },
      originalError
   ...
   * Handle HTTP status errors
   */
  private static handleHttpStatusError(status: number, operation: string, originalError: any): never {
    switch (status) {
      case 400:
        throw new CircleError(
          CircleErrorCode.VALIDATION_ERROR,
          `Bad request during ${operation}`,
      ...
      case 404:
        throw new CircleError(
          ...
          'Resource not found. Please try again or contact support.',
          false,
          undefined,
          { httpStatus: status },
          originalError
      ...
      case 504:
        throw new CircleError(
          CircleErrorCode.API_UNAVAILABLE,
      ...
      default:
        throw new CircleError(
          CircleErrorCode.UNKNOWN_ERROR,
          `HTTP ${status} error during ${operation}`,
          'An unexpected error occurred. Please try again or contact support if the problem persists.',
          true,
          30,
          { httpStatus: status },
 ...
 */
export class CircleRetryHandler {
  ...
  static async withRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `${operationName} failed (attempt ${attempt}/${maxRetries}), retrying in ${Math.round(jitteredDelay)}ms:`,
